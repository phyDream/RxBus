...
public class RxBusProxy<T> {
     ...
     * @param <V>
     */
    public <V> void subscribe(T host, String code, String tag, Scheduler scheduler, boolean isSticky, RxBus.OnReceivedListener<V> listener) {
        if (host == null || TextUtils.isEmpty(tag)) {
            return;
        }
        RxBus.subscribe(host, code, tag, scheduler, isSticky, listener);
    }
